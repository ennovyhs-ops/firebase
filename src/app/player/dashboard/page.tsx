...
export default function PlayerDashboard() {
  ...
  const conversations = React.useMemo(() => {
    return [...allConversations]
        .filter(c => c.timestamp && !isNaN(parseISO(c.timestamp).getTime()) && (c.recipient.includes("Everyone") || c.recipient.includes("All Players")))
                            ...
                            <div className="space-y-3">
                                {conversations.map(convo => (
                                    <div key={convo.id} className="text-sm">
                                        <Link href={`/player/messages?id=${convo.id}`} className="font-medium text-primary hover:underline leading-tight">
                                            {convo.subject}
                                        </Link>
